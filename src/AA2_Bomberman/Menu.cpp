...
#pragma region BUTTONS
	...
	Renderer::GetInstance()->LoadRect(T_LEVEL1, { SCREEN_WIDTH / 2 - vtmp.x / 2 - 150, 220, vtmp.x, vtmp.y });
	...
	Renderer::GetInstance()->LoadRect(T_LEVEL2, { SCREEN_WIDTH / 2 - vtmp.x / 2 + 150, 220, vtmp.x, vtmp.y });

	//RANKING BUTTON
	vtmp = Renderer::GetInstance()->LoadTextureText(F_GAMEOVER, Text{ T_RANKING_N, "Ranking", {0,0,0,255}, 0, 0 });
	Renderer::GetInstance()->LoadTextureText(F_GAMEOVER, Text{ T_RANKING_H, "Ranking", {200,100,0,255}, 0, 0 });
	textureRanking = T_RANKING_N;
	Renderer::GetInstance()->LoadRect(T_RANKING, { SCREEN_WIDTH / 2 - vtmp.x / 2 - 150, 330, vtmp.x, vtmp.y });
	...
	vtmp = Renderer::GetInstance()->LoadTextureText(F_GAMEOVER, Text{ T_SOUND_OFF_N, "Sound off", {0,0,0,255}, 0, 0 });
	Renderer::GetInstance()->LoadTextureText(F_GAMEOVER, Text{ T_SOUND_OFF_H, "Sound off", {200,100,0,255}, 0, 0 });
	textureSound = T_SOUND_ON_N;
	Renderer::GetInstance()->LoadRect(T_SOUND, { SCREEN_WIDTH / 2 - vtmp.x + 280, 330, vtmp.x, vtmp.y });

	//EXIT
	vtmp = Renderer::GetInstance()->LoadTextureText(F_GAMEOVER, Text{ T_EXIT_N, "Exit", {0,0,0,255}, 0, 0 });
...
void Menu::Update(InputData* _input)
{
	...
	if (Collisions::ExistCollision(_input->GetMouseCoords(), Renderer::GetInstance()->GetRect(T_LEVEL1)))
	{
		...
		if (_input->JustPressed(EInputKeys::MOUSE_LEFT)) { SetSceneState(ESceneState::CLICK_PLAY); SetCurrentLevel("1"); }
	}
	...
	if (Collisions::ExistCollision(_input->GetMouseCoords(), Renderer::GetInstance()->GetRect(T_LEVEL2)))
	{
		...
		if (_input->JustPressed(EInputKeys::MOUSE_LEFT)) { SetSceneState(ESceneState::CLICK_PLAY); SetCurrentLevel("2");
		}
	}
