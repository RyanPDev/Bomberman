...
void Ranking::Draw()
{
	...
	{
		VEC2 vtmp = Renderer::GetInstance()->UpdateTextureText(F_GAMEOVER, Text{ T_PL_NAMES, rank[i].name, {0, 0, 0, 255}, 0, 0 });
		...
		vtmp = Renderer::GetInstance()->UpdateTextureText(F_GAMEOVER, Text{ T_PL_SCORES, std::to_string(rank[i].score), {0, 0, 0, 255}, 0, 0 });
		...
		vtmp = Renderer::GetInstance()->UpdateTextureText(F_GAMEOVER, Text{ T_PL_RANK, std::to_string(i + 1), {0, 0, 0, 255}, 0, 0 });
